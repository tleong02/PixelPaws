...
class PredictTab:
    ...
    def _predict_thread(self):
        ...
        try:
            ...
            self.results_text.insert(tk.END, "PixelPaws Prediction\n")
            self.results_text.insert(tk.END, "=" * 60 + "\n\n")
            ...
            self.results_text.insert(tk.END, f"Classifier: {os.path.basename(clf_path)}\n")
            self.results_text.insert(tk.END, f"Video: {os.path.basename(video_path)}\n")
            self.results_text.insert(tk.END, f"DLC File: {os.path.basename(dlc_path)}\n")
            if features_path:
                self.results_text.insert(tk.END, f"Features: {os.path.basename(features_path)}\n")
            if dlc_config_path:
                self.results_text.insert(tk.END, f"DLC Config: {os.path.basename(dlc_config_path)}\n")
            ...
            self.results_text.insert(tk.END, f"  Behavior: {behavior_name}\n")
            self.results_text.insert(tk.END, f"  Threshold: {best_thresh:.3f}\n\n")
            
            # Check for DLC crop
            crop_x_offset = 0
            crop_y_offset = 0
            if dlc_config_path and os.path.isfile(dlc_config_path):
                self.results_text.insert(tk.END, "Checking DLC crop parameters...\n")
                try:
                    ...
                    if config.get('cropping', False):
                        ...
                        self.results_text.insert(tk.END, "  No cropping in config\n\n")
                except Exception as e:
                    self.results_text.insert(tk.END, f"  ⚠️  Could not read config: {e}\n\n")
            ...
            if features_path and os.path.isfile(features_path):
                self.results_text.insert(tk.END, "Loading pre-extracted features...\n")
                try:
                    ...
                    self.results_text.insert(tk.END, f"  ✗ Error loading: {e}\n")
                    self.results_text.insert(tk.END, "  Falling back to extraction...\n\n")
                    features_loaded = False
            
            # Extract if needed
            if not features_loaded:
                self.results_text.insert(tk.END, "Extracting features...\n")
                self.results_text.insert(tk.END, "  (This may take several minutes)\n")
            ...
            # Predict
            self.results_text.insert(tk.END, "Running classifier...\n")
            y_proba = predict_with_xgboost(model, X)
            y_pred = (y_proba >= best_thresh).astype(int)
            
            # Apply bout filtering
            if 'min_bout' in clf_data:
                self.results_text.insert(tk.END, "Applying bout filtering...\n")
            ...
            self.results_text.insert(tk.END, "="*60 + "\n")
            self.results_text.insert(tk.END, f"Total frames: {n_frames}\n")
            self.results_text.insert(tk.END, f"Behavior detected: {n_positive} frames ({100*n_positive/n_frames:.1f}%)\n")
            self.results_text.insert(tk.END, f"Behavior time: {behavior_time:.1f} seconds ({behavior_time/60:.1f} minutes)\n")
            self.results_text.insert(tk.END, "="*60 + "\n")
    ...
    def _save_outputs(self, video_path, video_dir, behavior_name, y_pred, y_proba, fps):
        ...
        if self.pred_save_csv.get():
            ...
            self.results_text.insert(tk.END, f"  ✓ Saved CSV: {os.path.basename(csv_path)}\n")
